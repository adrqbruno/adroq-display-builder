import { FONTS, ANIMS } from "./constants.js";
...
function textInput(label,value,onChange) {
  ...
  const i=el("input",{type:"text",class:"ctrl-input",value});
...
function selectField(label,value,options,onChange) {
  ...
  options.forEach(o=>{const opt=el("option",{value:o},o);if(o===value)opt.selected=true;s.appendChild(opt);});
...
function colorPick(label,value,onChange) {
  ...
  r.appendChild(inp); r.appendChild(hex); w.appendChild(r); return w;
}
function slider(label,value,min,max,onChange) {
  const w=el("div",{style:"margin-bottom:10px"});
  const lb=lbl(`${label}: ${value}`);
  const inp=el("input",{type:"range",min,max,value,style:"width:100%;accent-color:#111"});
  inp.addEventListener("input",e=>{lb.textContent=`${label}: ${e.target.value}`;onChange(Number(e.target.value));});
...
function imageUpload(label,current,onUpload) {
  ...
  inp.onchange=e=>{const f=e.target.files[0];if(!f)return;const rd=new FileReader();rd.onload=ev=>{ok.textContent="✓ Loaded";btn.textContent="Replace";onUpload(ev.target.result);};rd.readAsDataURL(f);};
  r.appendChild(btn);r.appendChild(ok);r.appendChild(inp);w.appendChild(r); return w;
}

function upd(layer,key,val) { state.layers[layer][key]=val; renderPreview(state.mode==="animated"); }

function panelBg() {
  const l=state.layers,f=document.createDocumentFragment();
  f.appendChild(section("Background",
    colorPick("Color",l.bg.color,v=>upd("bg","color",v)),
    imageUpload("Background Image",l.image.src,v=>upd("image","src",v)),
    ...(l.image.src?[
      slider("Opacity",l.image.opacity,0,100,v=>upd("image","opacity",v)),
      row(slider("X%",l.image.x,0,100,v=>upd("image","x",v)),slider("Y%",l.image.y,0,100,v=>upd("image","y",v))),
      row(slider("W%",l.image.w,0,100,v=>upd("image","w",v)),slider("H%",l.image.h,0,100,v=>upd("image","h",v))),
    ]:[])
  )); return f;
}
function panelLogo() {
  ...
  f.appendChild(section("Logo",
    imageUpload("Logo file",l.logo.src,v=>upd("logo","src",v)),
    ...(l.logo.src?[
...
function panelVisual() {
  ...
  f.appendChild(section("Visual Element",
    imageUpload("Image / PNG / PSD export",l.visual.src,v=>upd("visual","src",v)),
    ...(l.visual.src?[
      slider("Opacity",l.visual.opacity,0,100,v=>upd("visual","opacity",v)),
      xySliders("visual",l.visual.x,l.visual.y),
      row(numInput("W (px)",l.visual.w,v=>upd("visual","w",v)),
          numInput("H (px)",l.visual.h,v=>upd("visual","h",v))),
    ]:[])
  )); return f;
}
function panelHeadline() {
  ...
  f.appendChild(section("Headline",
    textInput("Text",l.headline.text,v=>upd("headline","text",v)),
    row(colorPick("Color",l.headline.color,v=>upd("headline","color",v)),numInput("Size (px)",l.headline.size,v=>upd("headline","size",v))),
    selectField("Font",l.headline.font,FONTS,v=>upd("headline","font",v)),
    xySliders("headline",l.headline.x,l.headline.y),
    selectField("Animation",l.headline.anim,ANIMS,v=>upd("headline","anim",v)),
  )); return f;
}
function panelBody() {
  const l=state.layers,f=document.createDocumentFragment();
  f.appendChild(section("Body Copy",
    textInput("Text",l.body.text,v=>upd("body","text",v)),
    row(colorPick("Color",l.body.color,v=>upd("body","color",v)),numInput("Size (px)",l.body.size,v=>upd("body","size",v))),
    selectField("Font",l.body.font,FONTS,v=>upd("body","font",v)),
    xySliders("body",l.body.x,l.body.y),
    selectField("Animation",l.body.anim,ANIMS,v=>upd("body","anim",v)),
  )); return f;
}
function panelCta() {
  const l=state.layers,f=document.createDocumentFragment();
  f.appendChild(section("CTA Button",
    ...
    selectField("Animation",l.cta.anim,ANIMS,v=>upd("cta","anim",v)),
  )); return f;
}

export const PANELS = { bg:panelBg, logo:panelLogo, visual:panelVisual, headline:panelHeadline, body:panelBody, cta:panelCta };
